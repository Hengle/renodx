...
struct PipelineShaderDetails {
  ...
  std::vector<reshade::api::format> render_targets;
  ...
  PipelineShaderDetails(
    ...
    for (uint32_t i = 0; i < subobject_count; ++i) {
      ...
      uint32_t replacement_shader_hash;
      std::optional<std::uint8_t> replacement_shader;
      ...
      if (auto pair = shader_replacements_inverse.find(shader_hash);
        ...
        replacement_shader_hash = shader_hash;
        shader_hash = pair->second;
        this->initialized_replacement = true;
      } else {
        replacement_shader_hash = 0u;
      ...
      this->subobject_shaders.push_back({
          ...
          .replacement_shader_hash = replacement_shader_hash,
          .stage = stage,
      });
...
struct __declspec(uuid("908f0889-64d8-4e22-bd26-ded3dd0cef77")) DeviceData {
  ...
  bool use_replace_on_bind = false;
...
struct __declspec(uuid("8707f724-c7e5-420e-89d6-cc032c732d2d")) CommandListData {
  ...
  reshade::api::pipeline_layout raytracing_pipeline_layout = {0};
  std::array<StageState, 3> stage_states = {};
  DeviceData* device_data = nullptr;
  bool replace_on_bind = false;
};
...
void ApplyReplacement(reshade::api::command_list* cmd_list, StageState& state) {
...
static bool BuildReplacementPipeline(
  ...
  if (details.subobject_shaders.empty()) return false;
...
static void OnInitCommandList(reshade::api::command_list* cmd_list) {
  ...
  data.device_data = &cmd_list->get_device()->get_private_data<DeviceData>();
}
...
static void OnBindDescriptorTables(reshade::api::command_list* cmd_list,
  ...
  auto& data = cmd_list->get_private_data<CommandListData>();
  bool applied = false;
  if ((reshade::api::shader_stage::all_graphics & stages) != 0u) {
    if (data.graphics_pipeline_layout != layout) {
      data.stage_states[VERTEX_INDEX].shader_hash = 0u;
      data.stage_states[PIXEL_INDEX].shader_hash = 0u;
      data.graphics_pipeline_layout = layout;
    }
    applied = true;
  }
  if ((reshade::api::shader_stage::all_compute & stages) != 0u) {
    data.stage_states[COMPUTE_INDEX].shader_hash = 0u;
    data.compute_pipeline_layout = layout;
    applied = true;
  }
  if ((reshade::api::shader_stage::all_ray_tracing & stages) != 0u) {
    data.raytracing_pipeline_layout = layout;
    applied = true;
  }
...
static reshade::api::pipeline GetReplacementPipeline(
    reshade::api::command_list* cmd_list,
    CommandListData& cmd_list_data,
    reshade::api::pipeline& pipeline,
    reshade::api::pipeline_stage stage) {
  auto& device_data = *cmd_list_data.device_data;
...
#ifdef DEBUG_LEVEL_1
    std::stringstream s;
    s << "utils::shader::OnBindPipeline(Initializing replacement state: ";
    s << (void*)pipeline.handle;
    s << ")";
    reshade::log::message(reshade::log::level::debug, s.str().c_str());
#endif
    const std::unique_lock write_lock(device_data.mutex);
    BuildReplacementPipeline(cmd_list->get_device(), device_data, details_pair->second);
    details = details_pair->second;
  }

  // Use BindDescriptorTable
...
#ifdef DEBUG_LEVEL_2
      std::stringstream s;
      s << "utils::shader::OnBindPipeline(Replacing ";
      s << ", stage: " << stage;
      s << ", compatible: " << compatible_stage;
      s << ", pipeline: " << reinterpret_cast<void*>(pipeline.handle);
      s << " => " << reinterpret_cast<void*>(details.replacement_pipeline.handle);
      s << ")";
      reshade::log::message(reshade::log::level::debug, s.str().c_str());
#endif
...
static void OnBindPipeline(
  ...
  for (int i = 0; i < COMPATIBLE_STAGES_SIZE; ++i) {
    ...
    if (stages == reshade::api::pipeline_stage::all
      ...
      if (cmd_list_data == nullptr) {
        cmd_list_data = &cmd_list->get_private_data<CommandListData>();
      }
      found_compatible = true;
      cmd_list_data->stage_states[i].shader_hash = 0u;
      cmd_list_data->stage_states[i].pipeline = pipeline;
      cmd_list_data->stage_states[i].stage = stages;
    }
  }
  ...
  if (!cmd_list_data->replace_on_bind) return;

  auto replacement_pipeline = GetReplacementPipeline(cmd_list, *cmd_list_data, pipeline, stages);
  if (pipeline.handle != 0u) {
    cmd_list->bind_pipeline(stages, replacement_pipeline);
  }
}
...
inline std::span<uint8_t> GetShaderData(const reshade::api::pipeline_subobject& subobject) {
  const reshade::api::shader_desc desc = *static_cast<const reshade::api::shader_desc*>(subobject.data);
  if (desc.code_size == 0) return {};
  return {
      (uint8_t*)(desc.code),
      (uint8_t*)(desc.code) + desc.code_size};
}
...
inline std::optional<std::vector<uint8_t>> GetShaderData(reshade::api::device* device, reshade::api::pipeline pipeline, uint32_t shader_hash) {
  ...
  if (auto details_pair = data.pipeline_shader_details.find(pipeline.handle);
    ...
    for (const auto& info : details.subobject_shaders) {
      ...
      auto& subobject = details.subobjects[info.index];
      const reshade::api::shader_desc desc = *static_cast<const reshade::api::shader_desc*>(subobject.data);
      if (desc.code_size == 0) return {};
